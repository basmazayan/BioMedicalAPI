...
namespace BiomedicalSystemAPI.Repositories.EquipmentReposatories
{
    public interface IEquipmentRepository : IDisposable
    {
        ...
        Task<ActionResult<IEnumerable<Assets>>> ToList();
        ActionResult<List<Assets>> GetAllEquimentsByeqCoverage(int eqCoverageId);
        ActionResult<List<Assets>> GetoneEquipmentBymaster(int masterId);
